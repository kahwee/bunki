...
export function startServer(
  ...
  fs.access(outputDir)
    .then(() => {
      ...
      Bun.serve({
        ...
        async fetch(req) {
          try {
            ...
            return new Response(
              `<h1>500 Server Error</h1><pre>${error}</pre>`,
              {
                status: 500,
                headers: { "Content-Type": "text/html" },
              },
      ...
      console.log(
        `Bunki development server running at http://localhost:${port}/`,
      );
    })
    .catch((error) => {
      console.error(
        `Error: Output directory ${outputDir} does not exist or is not accessible.`,
      );
